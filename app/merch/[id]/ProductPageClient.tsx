...
const COLORS = [
  { id: "black" as const, label: "Schwarz", hex: "#141414", border: "rgba(255,255,255,0.25)" },
  { id: "white" as const, label: "Weiß",   hex: "#EFEFEF", border: "rgba(255,255,255,0.5)" },
];
...
function getShirtImage(imageUrl: string | null, colorId: "black" | "white"): string {
  if (!imageUrl) {
    return colorId === "black" ? "/merch-shirt-black-front.png" : "/merch-shirt-white-front.png";
  }
  const base = imageUrl.replace(/\.(png|jpe?g)$/, "");
  if (base.includes("product-shirt-")) {
    return `${base}-${colorId}.png`;
  }
  return imageUrl;
}
...
export default function ProductPageClient({ product }: { product: Product }) {
  ...
  const imgSrc = isShirt
    ? getShirtImage(product.image_url, selectedColor)
                    ...
                    <thead>
                      <tr>
                        {["Größe", "Körperlänge", "Brustumfang (½)"].map((h) => (
                          <th
                            key={h}
                            style={{ padding: "0.4rem 0.5rem", textAlign: "left", color: "var(--primary)", letterSpacing: "0.06em", fontSize: "0.65rem", textTransform: "uppercase", borderBottom: "1px solid rgba(230,34,140,0.2)" }}
                          >
                            {h}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {SIZE_CHART.map((row, i) => (
                        <tr key={row.size} style={{ background: i % 2 === 0 ? "rgba(255,255,255,0.03)" : "transparent" }}>
                          <td style={{ padding: "0.35rem 0.5rem", color: selectedSize === row.size ? "var(--primary)" : "rgba(255,255,255,0.8)", fontWeight: selectedSize === row.size ? 700 : 400 }}>
                            {row.size}
                          </td>
